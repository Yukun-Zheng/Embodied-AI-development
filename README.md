...
[![Executable textbook regression](https://github.com/Yukun-Zheng/Embodied-AI-development/actions/workflows/minimal-code-regression.yml/badge.svg)](https://github.com/Yukun-Zheng/Embodied-AI-development/actions/workflows/minimal-code-regression.yml)
...
- **[Runnable Labs](labs/runnable/README.md)** — 已进入永久 CI 的机制实验；当前覆盖 Active Perception、Tactile Reflex、Async Execution、VLA Visual Intervention、Cross-Embodiment、Long-Horizon Memory、World Model MPC、Reasoning Negative Control、Continual Learning、Bimanual Coordination、Multi-Robot Collaboration、Safety Shield
...
其中 [`labs/runnable/`](labs/runnable/) 把选定 Lab 落成统一 executable contract。当前 12 个 reference labs 已进入永久 CI：
...
→ probe availability → paired visual intervention → action sensitivity / nuisance invariance → causal visual use

Lab 26 Cross-Embodiment Transfer
→ interface semantics → seen robot IDs → held-out morphology → zero-shot physical control

Lab 27 Long-Horizon Memory
→ one-shot cue → bounded/persistent state → delayed decision → memory cost

Lab 29 World Model MPC
→ passive prediction → counterfactual → horizon bias → control utility
...
→ endpoint state → midpoint / relative separation → differential feedback → shared-object strain / internal load

Lab 38 Multi-Robot Collaboration
→ heterogeneous capability → heartbeat freshness → coordinated allocation → failure reallocation

Lab 40 Watchdog / Safety Shield
→ fault / stale state → watchdog / predictive stopping → braking outcome → auditability / availability
```
...
- **Executable textbook regression** — 13 个 minimal examples + runnable Lab 的 deterministic mechanism smoke tests；
- **Textbook QA** — Part 0–50 连续性、TOC、关键资产、本地链接、51/51 Chapter 的 6/6 publication baseline，以及 Class-A local primary-evidence gate；
- **Website Build** — publication staging、MathJax/Mermaid 构建、关键正文/源码/Research Atlas/Runnable Labs 页面 smoke test。
...
│   └── CORE_DIAGRAMS.md
├── code/
│   └── minimal/                # 13 CPU minimal examples
├── case-studies/               # 7 canonical source/mechanism deep dives
├── labs/
│   ├── README.md
│   ├── LABS.md                 # 40 Labs + 3 Capstones
│   ├── EXECUTION_MATRIX.md     # CPU / simulator / real-robot layering
│   ├── EXPERIMENT_PROTOCOL.md
│   └── runnable/               # CI-verified reference mechanism labs
├── references/
...
│   ├── REFERENCES.md
│   └── BIBLIOGRAPHY.bib
├── scripts/                     # TOC / QA / publication / audit tools
└── .github/workflows/           # 5 permanent workflows
```
...
- **13** 个最小可执行脚本 + **12** 个 CI-verified runnable reference labs；
...
- **36 周**系统课程；
- Notation / Concept Index / Dependency Graph / Research Crosswalk；
...
- executable-textbook / textbook-QA / website-build 三层主 CI；
